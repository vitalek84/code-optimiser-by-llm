...
def main():
    ...
    data = []  # I added array data to gather data in parallel
    ...
    sorted_data = sorted(data, key=lambda x: len(x[0]), reverse=True)  # Sort data by length for multithreading in futures

    for df, symbol in sorted_data:
        df[['date', 'close', 'sma_10', 'sma_50']].to_parquet(f'for symbol {symbol}.parquet')  # To write df to parquet file
