...
The previous script is not optimised. We can use the Dask framework for parallel computations, multithreading and multiprocessing in Python. Dask.dataframe is used for dataframe computations with NumPy and Pandas. Dask encapsulates parallelism and asynchronous computations and automatically scales tasks to many cores. This allows to speed up python computations in case of highly parallelizable workloads. Let’s try to replace Pandas functionality with Dask in order to optimize the code.
```
...
def fetch_ohlcv(exchange, symbol, timeframe='1d', limit=100):
    ...
    try:
        ...
        return dd.DataFrame(ohlcv, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
