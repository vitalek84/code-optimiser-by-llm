import numpy as np
import pandas as pd
from datetime import datetime
from math import nan


def fetch_ohlcv(exchange, symbol, timeframe='1d', limit=100):
    """Fetch OHLCV data for a given symbol."""
    try:
        df = exchange.fetch_ohlcv(symbol, timeframe, limit=limit)
        return df
    except Exception as e:
        print(f"Error fetching data for {symbol}: {e}")
        return None


def calculate_sma(df, period):
    """Calculate Simple Moving Average (SMA) for the close prices."""
    return np.where(df['close'].rolling(window=period).mean().isnull(), nan, df['close'].rolling(window=period).mean())


def convert_to_timestamp(df):
    """Convert UNIX timestamp to human-readable date."""
    seconds = df['timestamp'] / 1000
    return np.where(pd.to_datetime(seconds, unit='s').dt.strftime('%Y-%m-%d').isnull(), nan,
                    pd.to_datetime(seconds, unit='s').dt.strftime('%Y-%m-%d'))


def filter_by_date(df, end_date):
    """Filter the OHLCV data to include rows up to the specified end_date."""
    return df[df['timestamp'] <= pd.Timestamp(end_date)].reset_index(drop=True)


def main():
    # Initialize exchange (Binance in this example)
    exchange = ccxt.binance({
        'rateLimit': 1200,
        'enableRateLimit': True,
    })

    # Fetch the top 20 coins by market cap (manually selected for simplicity)
    top_20_coins = [
        "BTC/USDT", "ETH/USDT", "BNB/USDT", "XRP/USDT", "DOGE/USDT",
        "ADA/USDT", "SOL/USDT", "DOT/USDT", "MATIC/USDT", "SHIB/USDT"
    ]

    # Restrict data till this date
    end_date = "2024-11-25"

    for symbol in top_20_coins:
        df = fetch_ohlcv(exchange, symbol)
        if df is not None:
            df['timestamp'] = df['timestamp'].astype(np.int64)
            df['timestamp'] = convert_to_timestamp(df)
            df = filter_by_date(df, end_date)

            if df.empty:
                print(f"No data available for {symbol} up to {end_date}.")
                continue

            df['sma_10'] = calculate_sma(df, 10)
            df['sma_50'] = calculate_sma(df, 50)

            print(f"SMA for {symbol} (up to {end_date}):")
            print(df[['timestamp', 'close', 'sma_10', 'sma_50']].tail(1))
        else:
            print(f"Could not calculate SMA for {symbol}.")


if __name__ == "__main__":
    main()
