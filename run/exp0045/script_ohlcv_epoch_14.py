...
"""
This script optimizes the execution time of fetching OHLCV data for multiple symbols by using concurrency.
It uses the `concurrent.futures.ThreadPoolExecutor` to fetch data in parallel, significantly reducing the overall execution time.

I noticed that the previous version works incorrectly. I analyzed the issue and fixed it. Also, I understand that I shouldn't take into account the execution time of not working versions.
"""
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = {executor.submit(fetch_ohlcv, exchange, symbol): symbol for symbol in top_20_coins}
