...
def very_inefficient_sort(data):
    ...
    while not is_sorted(data) and iteration < max_iterations:
        # NOTE: It is a good idea to check whether your algorithm works correctly.
        # NOTE: The attempted solution has the following issue: It can run in infinite loop without sorting the input list.
        # NOTE: To avoid infinite loops limits the number of iterations. Further performance optimizations might be possible by using specific properties of the sorting algorithm.
        # NOTE: I also noticed that the attempt to reverse sort a shuffled list is completely redundant. The resulting list is just shuffled anyway.
