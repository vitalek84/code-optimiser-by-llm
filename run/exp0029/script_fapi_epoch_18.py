...
"""
The previous version has an error. We were asked to fix the issue. After analyzing, we see that the exception is being caused because while iterating through the nested dict. the function is trying to access an index that doesn't exist. To fix this, a check has been added to see if the key exists before accessing it.
"""
        ...
        for level1_key, level1_value in data.items():
            for level2_key, level2_value in level1_value.items():
              ...
              #  Store data in the internal dictionary
                            if f"{level1_key}.{level2_key}.{level3_key}.{level4_key}" in internal_dict:
